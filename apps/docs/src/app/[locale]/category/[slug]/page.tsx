...
export async function generateMetadata({
    ...
    const path = `/category/${parameters.slug}`;
...
export default async function LocalizedCategoryArchive({
    params,
    searchParams
}: {
    params: Promise<{ locale: string; slug: string }>;
    ...
    const category = await taxonomy(database).categoryBySlug(parameters.slug);
    ...
    const [entries, settings] = await Promise.all([
        publicPostPage(() =>
            postSummaries(database).list({ categoryId: category.id, locale: context.locale, limit: 20, cursor })
        ),
        publicSiteSettings()
    ]);
    const path = localizedPath(context.locale, `/category/${category.slug}`);
    ...
    return (
        <main id="main-content" className="mx-auto min-h-[70dvh] max-w-4xl px-4 py-14 sm:px-6 sm:py-20">
            <header className="mb-12 border-b pb-10">
                ...
                <p className="text-muted-foreground mt-5 text-sm font-medium">{messages.category}</p>
                <h1 className="mt-2 text-4xl font-semibold tracking-tight text-balance sm:text-5xl">{category.name}</h1>
            </header>
            <ProgressivePostList
                key={listing.pageAddress}
                initialPage={listing}
                emptyMessage={messages.categoryEmpty}
                locale={context.locale}
            />
        </main>
    );
