...
- **Production initialization**: build successfully on Linux or WSL, apply `db:migrate:remote`, deploy the built Worker, then run `db:docs:migrate:remote` and open `/setup` on the deployed address. The `develop` workflow builds before advancing the production schema, then reuses that artifact for deployment.
