...
Summary listings page by keyset rather than by offset. The cursor carries the last row's publication moment and Entry identifier opaquely, and the next page continues strictly after that pair in the same order. An offset skips rows by counting them, so publishing an Entry between two requests shifts every later page and makes a reader see a repeated or missing Entry; a keyset names the position instead of counting to it, and also lets SQLite stop at the requested rows rather than walking the ones it discards. A page reports a following cursor only when the reader looked one row past the limit and found one, so the last page never offers an address that leads nowhere. An unreadable or forged cursor is refused rather than silently treated as the first page, since a public list address that quietly changes meaning is worse than one that says it is invalid.
